...
export const TelemetryService = {
	...
	async getCountsByMachine(): Promise<MachineCount[]> {
		const { data } = await ApiClient.client.get(
			'/api/telemetry/count/by-machine'
		)
		return data
	},
...
export const AlertService = {
	...
	async getAcknowledgedAlerts(
		...
		const { data } = await ApiClient.client.get(
			`/api/alerts/acknowledged?limit=${limit}&offset=${offset}`
		)
		return data
	},
	...
	async getUnacknowledgedAlerts(
		...
		const { data } = await ApiClient.client.get(
			`/api/alerts/unacknowledged?limit=${limit}&offset=${offset}`
		)
		return data
	},
	...
	async getAlertSeverityCounts(
		days: number = 1
	): Promise<AlertSeverityCountsResponse> {
		const { data } = await ApiClient.client.get(
			`/api/alerts/severity-counts?days=${days}`
		)
		return data
	},
