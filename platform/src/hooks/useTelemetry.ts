...
export const useTelemetry = () => {
	...
	const [alertCounts, setAlertCounts] = useState<AlertCountsResponse | null>(
		null
	)
	const [topOffenders, setTopOffenders] = useState<TopOffendersResponse | null>(
		null
	)
	...
	const fetchRecentAlerts = useCallback(async () => {
		try {
			...
			const recentAlerts: any = await AlertService.getRecentAlerts(5)
